...
static void bench_client_change(const string &hostname, unsigned int key_size, unsigned int iterations, unsigned int n_threads)
{
    try
    {
#ifdef BENCHMARK
...
int main(int argc, char* argv[])
{
    if (argc != 4)
    {
        std::cerr << "Usage: bench_client_change <host> <key_size> <iterations>" << std::endl;
        return 1;
    }
    ...
    bench_client_change(hostname, key_size, iterations, n_threads);
